...
## Merge TidyTuesday Data

The Tidy Tuesday version of these data have useful information about the colleges in the `tuition_cost` dataset. All we want from this dataset is the type of university and the degree length the college is. The costs listed in these data are for a single year only, which isn't useful when we want to examine trends over time

```{r load tuition_cost data}
# read in the data and only keep the variables that are a character data type
tuition_cost <- readr::read_csv('https://raw.githubusercontent.com/rfordatascience/tidytuesday/master/data/2020/2020-03-10/tuition_cost.csv') %>%
  select(where(is.character))
```

```{r merge tuition_cost with cost_attendance}
cost_attendance <- tuition_cost %>%
  right_join(cost_attendance, by = c('name','state_code'))
```
...
```{r}
write_csv(cost_attendance,'../cleaned_data/cost_of_attendance.csv')
